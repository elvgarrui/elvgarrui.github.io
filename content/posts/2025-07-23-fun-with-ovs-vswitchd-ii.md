...
dministrators to check the logs using this:
```sh
oc get logs -c ovs-vswitchd $(oc get pod -l service=ovn-controller-ovs -o name)
```

## The first try 
The most direct approach was to sort around the detach by dettaching the command using `&` instead
of `--detach`. That way we are "dettaching" it to a different execution thread, allowing the process
...
This will make us lose the initial part of the ovs-vswitchd logging in the stdout (because we are using tail),
and an extra file is generated in the container, which is something to be avoided if possible. __But it works!__
...
were the podified OSP env was running (deployed using [install_yamls](https://github.com/openstack-k8s-operators/install_yamls)).
The environment needed to have:
- __At least one EDPM (External Data Plane Management) node__ serving as a compute node.
- __An OSP deployed VM running on the EDPM node being pinged__ to test possible packet loss.
- The OSP environment needs to have __centralized FIPs__ (Floating IPs) in order to ensure
